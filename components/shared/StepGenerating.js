...
export default function StepGenerating({ formData, user, onBack }) {
  ...
  const runGeneration = async () => {
    for (let i = 0; i < 2; i++) {
      setStageIdx(i); setProgress(STAGES[i].pct)
      await delay(700)
    }
    try {
      setStageIdx(2); setProgress(STAGES[2].pct)
      ...
      setStageIdx(5); setProgress(STAGES[5].pct)
  ...
  const retry = () => {
    ...
    setError(''); setStageIdx(0); setProgress(0); setDone(false); setGenerated(null)
    hasRun.current = true
    runGeneration()
  }
      ...
      {error ? (
        <div>
          ...
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center' }}>
            <button onClick={onBack} style={{ padding: '12px 24px', background: 'var(--surface)', border: '1px solid var(--border2)', borderRadius: 50, color: 'var(--text2)', cursor: 'pointer', fontFamily: 'Outfit', fontSize: 14 }}>← Go Back</button>
            <button className="btn-primary" onClick={retry} style={{ padding: '12px 28px', fontSize: 14 }}>Try Again</button>
          </div>
        </div>
      ) : (
        <>
          <div style={{ position: 'relative', width: 100, height: 100, margin: '0 auto 36px' }}>
            ...
            <div style={{ position: 'absolute', inset: 10, borderRadius: '50%', background: 'radial-gradient(circle, rgba(0,229,255,0.15), transparent)', border: '2px solid var(--cyan)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 28, animation: done ? 'none' : 'spin-slow 5s linear infinite' }}>
              {done ? '✓' : '✦'}
            </div>
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8, maxWidth: 380, margin: '0 auto' }}>
            {STAGES.slice(0, -1).map((s, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, opacity: i <= stageIdx ? 1 : 0.3, transition: 'opacity 0.4s' }}>
                <div style={{ width: 22, height: 22, borderRadius: '50%', flexShrink: 0, background: i < stageIdx ? 'rgba(0,229,255,0.15)' : 'var(--surface)', border: `1px solid ${i < stageIdx ? 'var(--cyan)' : 'var(--border2)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, color: 'var(--cyan)' }}>
                  {i < stageIdx ? '✓' : i === stageIdx ? <div style={{ width: 6, height: 6, borderRadius: '50%', background: 'var(--cyan)' }} /> : ''}
                </div>
                <span style={{ fontSize: 13, color: 'var(--text2)', textAlign: 'left' }}>{s.msg.replace('...', '')}</span>
              </div>
            ))}
          </div>
          {generated && (
            <div style={{ marginTop: 32, padding: '20px 24px', background: 'var(--cyan-dim)', border: '1px solid rgba(0,229,255,0.2)', borderRadius: 12, maxWidth: 420, margin: '32px auto 0', textAlign: 'left' }}>
